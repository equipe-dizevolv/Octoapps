...
-- Tarefas (Índices mais importantes para performance)
...
-- Função para criar notificação de prazo próximo (executar via cron job ou função agendada)
...
-- Políticas para CONTATOS (todos os usuários autenticados podem ver)
...
-- Políticas para TAREFAS (Mais importantes)
...
COMMENT ON TABLE tarefas IS 'Tabela principal de gestão de tarefas do sistema jurídico';
COMMENT ON TABLE projetos IS 'Projetos/Casos jurídicos';
COMMENT ON TABLE oportunidades IS 'Pipeline de vendas e oportunidades (CRM)';
COMMENT ON TABLE contatos IS 'Cadastro de clientes e contatos';
COMMENT ON TABLE comentarios IS 'Comentários e histórico de atividades';
