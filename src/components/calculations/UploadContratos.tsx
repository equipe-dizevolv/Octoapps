...
export function UploadContratos({ onNavigate }: UploadContratosProps) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Abre o dialog para selecionar o tipo de caso
    setIsDialogOpen(true);
  };

  const handleConfirmCaseType = () => {
    if (!selectedCaseType) {
      toast.error('Selecione o tipo de caso');
      return;
    }
    
    setIsDialogOpen(false);
    toast.success(`Caso do tipo "${selectedCaseType}" criado com sucesso!`);
  };
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="w-full">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={setSelectedCaseType} value={selectedCaseType}>
                      <SelectTrigger>
                        <SelectValue placeholder="Selecione o tipo de caso" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="Financiamento Imobiliário">Revisão de Financiamento Imobiliário</SelectItem>
                        <SelectItem value="Cartão de Crédito">Revisão de Cartão de Crédito</SelectItem>
                        <SelectItem value="Empréstimos e Financiamentos">Revisão Geral (Empréstimos e Financiamentos)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
        ...
        {/* Dialog para selecionar o tipo de caso */}
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Selecione o Tipo de Caso</DialogTitle>
              <DialogDescription>
                Escolha o tipo de caso que deseja criar com base nos dados extraídos.
              </DialogDescription>
            </DialogHeader>
            <Select onValueChange={setSelectedCaseType} value={selectedCaseType}>
            ...
            <DialogFooter>
              <Button onClick={handleConfirmCaseType} className="gap-2">
                Confirmar
                <ArrowRight className="w-4 h-4" />
              </Button>
            </DialogFooter>
