...
import type { UserPersona, ThemeMode } from '@/app/providers';
...
export function Topbar({ currentUser, onUserChange, theme, onThemeToggle, onMobileMenuToggle }: TopbarProps) {
  ...
  const users: UserPersona[] = ['Ana Admin', 'Diego Perito', 'Maria Advogada'];
          ...
          <DropdownMenuContent align="end">
            ...
            {users.map(user => (
              <DropdownMenuItem 
                ...
                className={currentUser === user ? 'bg-gray-100 dark:bg-gray-800' : ''}
              >
                {user}
                {currentUser === user && (
                  <span className="ml-auto text-xs text-blue-600 dark:text-blue-400">Atual</span>
                )}
              </DropdownMenuItem>
            ))}
            <DropdownMenuSeparator />
